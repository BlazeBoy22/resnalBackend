...
let post_headers = {
  ...
  Referer: "https://results.vtu.ac.in/JFRVEcbcs23/index.php",
...
async function getNewSession() {
  let url = "https://results.vtu.ac.in/JFRVEcbcs23/index.php";
  let headers = {
    "User-Agent":
  ...
  const path = "C:\\Users\\sunay\\Downloads\\RESNALNOV2022-main\\cap.png"
...
async function getResult(
  ...
  const url = "https://results.vtu.ac.in/JFRVEcbcs23/resultpage.php";
  var data = `Token=${post_payload.Token}&lns=${post_payload.lns}&captchacode=${post_payload.captchacode}`;
  var config: AxiosRequestConfig = {
    method: "post",
    url: "https://results.vtu.ac.in/JFRVEcbcs23/resultpage.php",
              ...
              $(element)
                ...
                .each((i, ele) => {
                  switch (i) {
                    ...
                    case 3:
                      result.old = parseInt($(ele).text().trim());
                    // case 4:
                    //   ($(ele).text().trim());
                    case 5:
                      result.new = parseInt($(ele).text().trim());
                    // case 6:
                    //   ($(ele).text().trim());
                    case 7:
                       result.final = parseInt($(ele).text().trim());
                    case 8:
...
(async () => {
  ...
  const json1: Array<{
    ...
    Batch: string;
    Sem: string;
  }> = await csv().fromFile("7threval.csv");
  ...
  fs.writeFile("result1.json", JSON.stringify(Result, null, 2), (e) => {
    if (e) throw e;
  });
  console.log("=========================");
